...
import com.beauty.refreshwidget.load.PlaneDrawable;
import com.beauty.refreshwidget.load.RefreshDrawable;
import com.beauty.refreshwidget.load.RefreshMode;


public class PullRefreshLoadPromptLayout extends ViewGroup
{
	private static final float DECELERATE_INTERPOLATION_FACTOR = 2f;
	private static final int INVALID_POINTER = -1;
	private static float DRAG_RATE = .16f;
	private static final int DRAG_MAX_DISTANCE_PULL = 45;
	private static final int DRAG_MAX_DISTANCE_LOAD = 250;
	private static float DRAG_RATE_PULL = .16f;
	private static float DRAG_RATE_LOAD = .4f;

	private View mTarget;
	private ImageView mRefreshView;
	private View mLoadPromptView;
	private View promptTextView;
	...
	//是否需要底部上拉弹出提示，默认设置不需要，这样就只有下拉刷新了
	public boolean isNeedLoadPrompt = false;
	// 不满屏时需不需要设置底部可上拉弹出提示
	public boolean isFullScreen = false;
	private int mLoadHeignt = 0;
	private int mLoadPromptOffset;

	public PullRefreshLoadPromptLayout(Context context)
	{
		this(context, null);
	}

	public PullRefreshLoadPromptLayout(Context context, AttributeSet attrs)
	{
		super(context, attrs);
		...
		mSpinnerFinalOffset = mTotalDragDistance = dp2px(DRAG_MAX_DISTANCE_PULL);

		mRefreshView = new ImageView(context);
		setRefreshDrawable(new PlaneDrawable(getContext()));
		mRefreshView.setVisibility(GONE);
		addView(mRefreshView, 0);

		mLoadPromptView = LayoutInflater.from(context).inflate(R.layout.match_down_pull_intro, null);
		promptTextView = mLoadPromptView.findViewById(R.id.tv_descDetail);
		promptTextView.post(new Runnable()
		{
			@Override
			public void run()
			{
				mLoadHeignt = promptTextView.getHeight();
				RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams)promptTextView.getLayoutParams();
				layoutParams.bottomMargin = -mLoadHeignt;
				promptTextView.setLayoutParams(layoutParams);
			}
		});

		setLoadDrawable();
		mLoadPromptView.setVisibility(GONE);
		addView(mLoadPromptView, 0);

		setWillNotDraw(false);
		ViewCompat.setChildrenDrawingOrderEnabled(this, true);
	}

	private void offsetLoadPromptTopAndBottom(int offset)
	{
		mLoadPromptOffset += offset;
		if(mLoadHeignt == 0)
		{
			mLoadHeignt = promptTextView.getHeight();
		}
		RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams)promptTextView.getLayoutParams();
		layoutParams.bottomMargin = -mLoadHeignt - mLoadPromptOffset;
		promptTextView.setLayoutParams(layoutParams);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		mLoadPromptView.measure(widthMeasureSpec, heightMeasureSpec);
	}
	...
	private void ensureTarget()
	{
		...
		if(getChildCount() > 0)
		{
			for(int i = 0; i < getChildCount(); i++)
			{
				...
				if(child != mRefreshView && child != mLoadPromptView) mTarget = child;
			}
		}
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev)
	{
		...
		switch(action)
		{
			...
			case MotionEvent.ACTION_MOVE:
				...
				if((mLastDirection == RefreshMode.PULL_FROM_START && yDiff < 0) || (mLastDirection == RefreshMode.PULL_FROM_END && yDiff > 0))
				{
					return false;
				}
	...
	public boolean onTouchEvent(MotionEvent ev)
	{
		...
		switch(action)
		{
			...
			case MotionEvent.ACTION_MOVE:
			{
				...
				if(!mIsBeingDragged && (yDiff > 0 && mLastDirection == RefreshMode.PULL_FROM_START) || (yDiff < 0 && mLastDirection == RefreshMode.PULL_FROM_END))
				{
					mIsBeingDragged = true;
				}
				...
				{
					//上拉刷新view和底部提示view的粘性和大小不同设置
					if(yDiff > 0)
					{
						DRAG_RATE = DRAG_RATE_PULL;
						mSpinnerFinalOffset = mTotalDragDistance = dp2px(DRAG_MAX_DISTANCE_PULL);
					}
					else
					{
						DRAG_RATE = DRAG_RATE_LOAD;
						mSpinnerFinalOffset = mTotalDragDistance = dp2px(DRAG_MAX_DISTANCE_LOAD);
					}

					final float scrollTop = yDiff * DRAG_RATE;// yDiff---viewGroup滑动的距离     DRAG_RATE---拉动弹性效果
					float originalDragPercent = scrollTop / mTotalDragDistance;// mTotalDragDistance--可拉出的最大距离
					...
					if(originalDragPercent < 0)
					{
						...
						if(isNeedLoadPrompt)
						{
							if(mLoadPromptView.getVisibility() != VISIBLE)
							{
								mLoadPromptView.setVisibility(VISIBLE);
							}
							setTargetOffsetTop(-targetY - mCurrentOffsetTop, true);
						}
					}
	...
	private void animateLoadOffsetToCorrectPosition()
	{
		...
		mLoadPromptView.clearAnimation();
		mLoadPromptView.startAnimation(mAnimateLoadToCorrectPosition);
	}
	...
	{
		if(refreshing && mRefreshListener != null)
		{
	...
	private Animation.AnimationListener mRefreshAnimationListener = new Animation.AnimationListener()
	{
		...
		public void onAnimationStart(Animation animation)
		{
			...
			if(mTarget != null)
			{
				mTarget.setPadding(0, 0, 0, dp2px(DRAG_MAX_DISTANCE_PULL));
			}
		}
	...
	private Animation.AnimationListener mRefreshLoadAnimationListener = new Animation.AnimationListener()
	{
		...
		public void onAnimationStart(Animation animation)
		{
			mLoadPromptView.setVisibility(View.VISIBLE);
		}
	...
	private void setTargetOffsetTop(int offset, boolean requiresUpdate)
	{
		...
		offsetLoadPromptTopAndBottom(offset);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b)
	{
		...
		mLoadPromptView.layout(left, top, left + width - right, top + height - bottom);
	}
