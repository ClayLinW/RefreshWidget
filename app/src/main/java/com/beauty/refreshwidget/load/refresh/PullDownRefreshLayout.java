...
import com.beauty.refreshwidget.load.PlaneLoadDrawable;
...
public class PullDownRefreshLayout extends ViewGroup
{
	...
	private static final int DRAG_MAX_DISTANCE = 45;
	...
	public boolean mNoLimitBorderSlide = false;
	...
	public PullDownRefreshLayout(Context context, AttributeSet attrs)
	{
		...
		setLoadDrawable(new PlaneLoadDrawable(getContext()));
	...
	public void setRecyclerView()
	{
		...
		recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener()
		{
			...
			public void onScrolled(final RecyclerView recyclerView, int dx, int dy)
			{
				...
				if(mScroolState == 1 && mIsStartSlide)
				{
					mIsStartSlide = false;
					if(mOffsetY <= 0)
					{
	...
	{
		if(!isEnabled() || (!canChildScrollUp() && !mRefreshing) && mScroolState == 1 && mCanSlide)
		{
			if(mFirstRecord)
			{
				mFirstRecord = false;
				mStartPoint = mInitialMotionY = ev.getY();
			}
		...
		switch(action)
		{
			...
			case MotionEvent.ACTION_MOVE:
			{
				...
				if(mRefreshing || mLoading)
				{
					...
					{
						// TODO: 2018/7/27 3.让滑倒最下面的时候，不要继续滑，避免把加载给消除掉，但是要给recyclerview加上一个个上拉加载view一样大小的footerview
						return true;
//                        targetY = -1;
//                        mInitialMotionY = y;
//                        mInitialOffsetTop = 0;
...
//                        }
	...
	private Animation.AnimationListener mRefreshAnimationListener = new Animation.AnimationListener()
	{
		...
		public void onAnimationStart(Animation animation)
		{
			...
			if(mTarget != null)
			{
				mTarget.setPadding(0, 0, 0, dp2px(DRAG_MAX_DISTANCE));
			}
		}
	...
	private Animation.AnimationListener mRefreshLoadAnimationListener = new Animation.AnimationListener()
	{
		...
		public void onAnimationStart(Animation animation)
		{
			...
			mTarget.setPadding(0, dp2px(DRAG_MAX_DISTANCE), 0, 0);
		}
